...
`Failed` is written only from `ProvisionStorefrontJob::failed()`, never from a catch inside `handle()`: an attempt that throws with retries left is still `Processing`. Marking it failed per-attempt made the panel show live deployments as dead and let `storefront:retry-failed` pick up rows with a job in flight.
