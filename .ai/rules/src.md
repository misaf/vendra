...
Never create a `User` with `User::query()->create()` or hash a user password with `Hash::make` outside `vendra-user`. Use `Misaf\VendraUser\Actions\CreateUserAction` — pass `tenant: null` for platform identities (console, reseller), which keeps `tenant_id` null even inside a tenant context — and `UpdateUserPasswordAction`, which handles tenant-less users. Do not add per-package copies (e.g. the removed `UpdateResellerUserPasswordAction`).
