...
A seeder must not run a command (`$this->command->call(...)`, `Artisan::call`). Commands are operator tools with prompts and CLI-only output. Inject the domain Action into the seeder's constructor and read shared values from config (e.g. `Config::string('vendra-console.default_email')`) instead of reaching into the command.
