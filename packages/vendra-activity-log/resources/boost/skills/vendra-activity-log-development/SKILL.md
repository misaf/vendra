...
- Add or update focused Pest coverage, then run `php artisan test --compact --testsuite=vendra-activity-log` and `composer stan`.
...
Treat `packages/vendra-activity-log` as the source of activity-log domain behavior and Filament admin UI.

- Use namespace `Misaf\VendraActivityLog`.
...
- Do not place activity-log domain code in the host app unless the host app is only integrating the module.
- Keep cross-module dependencies explicit in `composer.json`; do not introduce a dependency without approval.

## Domain Model Standards

Follow the existing `ActivityLog` patterns for new activity-log entities.
...
- `Listeners\LogSettingsActivity` listens to `SavingSettings` and logs a settings class that implements `ShouldLogActivity`: only the properties the save changes, with the stored values read from a fresh instance as `old`, under log name `settings` and the settings group as description, with no subject. A save without a current store writes the platform row and is logged as platform activity. Keep platform (global repository) settings classes off `ShouldLogActivity` for now: saved while a store is current, `BelongsToTenant` would stamp that store on their entry.
- `activity_log.tenant_id` is nullable and a null is platform activity, so `activity_log` stays out of `TenantTableRegistry` (the `vendra-tenant:enable` retrofit would backfill it and force NOT NULL). `LogModelActivity` records the current store when there is one; without one it records the store that owns the subject (the subject's tenant id, or the key of a tenant model itself) and leaves a tenantless subject as platform activity, so logging never fails the write in the console or reseller panels. The admin panel's tenant scope hides platform activity from stores.
...
Keep every resource that declares a `$cluster`, including its pages, schemas, tables, relation managers, actions, and resource-scoped widgets, under `src/Filament/Clusters/Resources/` with the matching `Misaf\VendraActivityLog\Filament\Clusters\Resources` namespace and plugin discovery path. Resources without a cluster belong under `src/Filament/Resources/`. Keep dashboard widgets in `src/Filament/Widgets`.
...
- Use this module's translation keys (`vendra-activity-log::attributes`, `vendra-activity-log::navigation`) for labels, breadcrumbs, and navigation.
...
- Keep navigation labels and groups configurable through the module `Plugin` and `config/vendra-activity-log.php`. Do not add a `tenant_aware` config value; tenant awareness derives from the bound `TenantResolver`.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets, plus an expectation that the module stays tenant-agnostic, e.g. `arch()->expect('Misaf\VendraActivityLog')->not->toUse('Misaf\VendraTenant')`.
- Add feature or Livewire tests when changing Filament behavior with meaningful user-visible effects.
- Run checks from the host app: `php artisan test --compact --testsuite=vendra-activity-log` and `composer stan`.
