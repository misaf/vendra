...
- `TenantEntitlements` (null default `NullTenantEntitlements`: everything allowed, nothing limited) answers what the plan behind a tenant allows through the `Enums\PlanFeature` flags and the `Enums\PlanLimit` per-store caps; `misaf/vendra-store` binds the real one. Domain packages register how much of a limit a tenant uses with `Tenancy\TenantUsageRegistry` (counters take the tenant explicitly and drop every global scope) and guard their writes with `assertCanAdd()`/`assertAllows()`, which throw `Exceptions\EntitlementExceededException` with a translated message panels can show as it is.
