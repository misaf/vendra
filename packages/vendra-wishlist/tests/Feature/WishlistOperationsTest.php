...
it('creates the default list for an owner on first use', function (): void {
    ...
    $first = Wishlist::firstOrCreateDefaultFor($user);
    $second = Wishlist::firstOrCreateDefaultFor($user);
...
it('keeps separate default lists for separate owners', function (): void {
    Wishlist::firstOrCreateDefaultFor(createTestUser());
    Wishlist::firstOrCreateDefaultFor(createTestUser());

    expect(Wishlist::query()->count())->toBe(2);
});

it('allows one default list per owner but any number of other lists', function (): void {
    $user = createTestUser();
    $owner = ['owner_type' => $user->getMorphClass(), 'owner_id' => $user->getKey()];

    WishlistFactory::new()->count(2)->create([...$owner, 'is_default' => false]);
    WishlistFactory::new()->create([...$owner, 'is_default' => true]);

    expect(fn () => WishlistFactory::new()->create([...$owner, 'is_default' => true]))
