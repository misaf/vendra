...
- A store has one primary domain (`is_primary`, read through `Store::primaryDomain()`); any other active domains are aliases that also resolve the store. The database allows only one live primary per store, so demote the current primary before promoting another.
- Add aliases through `Actions\AddStoreDomainAliasAction`, remove one through `Actions\RemoveStoreDomainAliasAction` (it keeps the alias as trashed history and refuses the primary), and promote one through `Actions\MakeStoreDomainPrimaryAction`; all three force a storefront redeploy, because the container's Traefik rule lists every active domain and labels cannot change in place.
- Replace the primary domain through `Actions\ReplaceStoreDomainAction`: its caller normalizes and validates the replacement (as `Filament\Actions\ReplaceDomainTableAction` does); the action retains the old primary as soft-deleted history, leaves aliases alone, creates the new active primary domain inside the store's own tenant context, and carry the storefront over — update the deployment's `domain` in the same transaction, then dispatch a forced `ProvisionStorefrontJob`. Routing lives in a container label that cannot be edited, so a domain change is a container replacement.
- `CompleteStoreProvisioningJob` keeps only queue concerns; the checkpointed provisioning steps live in `Actions\CompleteStoreProvisioningAction` and failures go through `Actions\FailStoreProvisioningAction`, which never downgrades a ready store. Each step saves its own checkpoint instead of sharing a transaction.
- Administrator-driven availability and failed provisioning recovery use `SuspendStoreAction`, `ReactivateStoreAction`, and `RetryStoreProvisioningAction`. Redeploy/retry use their domain actions and the existing deployment job/transition rules.
- Business removal uses `OffboardStoreAction`, which requires an audit reason and soft-deletes after inactivation; restoration uses `RestoreOffboardedStoreAction`, rechecks reseller quota, and respects provisioning readiness.
- `Support\StoreTenantEntitlements` binds the support `TenantEntitlements` from the store reseller's plan (console-owned stores are unrestricted). Custom domains (outside `*.{storefront.base_domain}`, `StoreDomain::isCustom()`) need `PlanFeature::CustomDomain` at create, alias and replace; aliases also respect `PlanLimit::DomainsPerStore` under a store row lock, and `AddDomainAliasTableAction` is disabled with the refusal as a tooltip at that limit. The panels catch `EntitlementExceededException` and show its message. Do not expose force deletion as an administrator workflow. `assertCanAdd()` locks the store row before counting, so creates inside a transaction cannot race past a limit.
...
- The billing reseller is optional and inverted: `stores.reseller_id` is a plain nullable indexed column, the reseller is fetched through `Contracts\StoreResellerResolver`, and `ProvisionStoreAction` / `CreateStorePage::resolveReseller()` take `(Model&SubscriptionSubscriber)|null`. A null reseller is a store the console owns directly, not an error.
- It depends on `misaf/laravel-docker-engine` for driver-managed Engine access and on `misaf/vendra-tenant` for the tenancy engine it plugs its `Store` into. It must **not** depend on `misaf/vendra-reseller`: a store's billing reseller is reached through `Contracts\StoreResellerResolver`, which the reseller package binds. Keep the arrow pointing reseller → store.
...
- `StorefrontImage` records the approved immutable image reference and optional notes. New deployments may select only active catalog entries; existing deployments keep using their selected entry after it is disabled. Never reintroduce global `STOREFRONT_IMAGE` configuration.
...
- Whether the platform is creating stores at all is `Settings\StoreCreationSettings` (group `store_creation`, `global` repository), read directly as `->open`. Console and reseller store creation both read it, so the shared rule lives below both panels.
...
- Cover the tenancy side too: Store acting as the current tenant, host resolution, `store_id` scoping of store domains, and the billing port defaulting to `Support\NullStoreResellerResolver`.
