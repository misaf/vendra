...
- `Console\Commands\Concerns\IdentifiesConsoleUser` normalizes `--email`/`--username` and looks the user up. Each command still owns its validation order, its messages, and returns before any write. The only translated CLI string is the identifier-required message (`vendra-console::commands.grant_requires_identifier` / `.revoke_requires_identifier`, in `resources/lang/*/commands.php`); every other command message is hardcoded English, as in every other package.
- Identifiers resolve separately: a username-only run does not fall back to the default email account, and both identifiers together must resolve to the same tenantless user: when one names nobody the run fails naming that identifier, and when they name different users it is rejected as mismatched. `user-grant` and `user-revoke` additionally require an identifier and reject a blank one; `user-password` falls back to `vendra-console.default_email` only when no identifier is given, and a blank `--email` is still rejected. `user-create` requires `--email` and never uses the default, which only the seeder assigns.
- `user-create` requires an explicit `--username`, passed to `CreateConsoleUserAction::execute($username, $email, $password)`; it never prompts for one. Validate its format with `UserRules::username()`; an email or username already held by a tenantless user fails without suffixes or retries and names only the sibling that applies (`user-password` when that user already has console access, `user-grant` otherwise), and `ConsoleSeeder` explicitly uses `console`.
...
- Supplied and generated passwords must pass `required` and `UserRules::password()`; explicitly empty or whitespace-only passwords fail. Generated ones come from `UserRules::generatePassword()`, which reads the same policy. Existing-user operations preserve the username.
